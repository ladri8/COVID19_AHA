...
revision <- filthosp[!(Reduce("|", lapply(filthosp[all.vars(fit4@call$formula)], is.na))), ]
dim(revision)
...
tabla <- select(revision, -c(CASE_ID, PATIENT_DISPLAY_ID, FACILITY_DISPLAY_ID, TROPADM, - TROPUADM,
...
print(tabla)
tabla$RACEgr <- fct_collapse(revision$RACEi,
                             UTD = "UTD",
