...
p2 <- byrace2 %>% filter((!(COVCLINTRIAL == "No"))) %>% ggplot(aes(y= percent, x= reorder(RACEi, -percent), fill=COVCLINTRIAL))+ 
  geom_bar(stat="identity", width = 0.8) + 
  ggtitle("Figure 1. Percentage of Enrollment in Clinical Trials") +
  xlab('Race/Ethnicity') + ylab('Percentage') +
   geom_text(aes(x = RACEi, label = paste0(rounded, '%')), 
...
p3 <- byrace2 %>% filter((!(COVCLINTRIAL == "No"))) %>% ggplot(aes(y= percent, x= reorder(raceplot, -percent), fill=COVCLINTRIAL)) +
  geom_bar(stat="identity", width = 0.8) +
  ggtitle("Figure 1. Percentage of Enrollment in Clinical Trials") +
  xlab('Race/Ethnicity') + ylab('Percentage') +
