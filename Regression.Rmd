...
#Table
minadj_tab<- minadj_filtered %>% tbl_regression(exponentiate = TRUE ) %>% bold_labels() %>% bold_p(t= 0.05) %>% add_nevent()

minadj_tab <- as_flex_table(minadj_tab, include = everything(),return_calls = FALSE, strip_md_bold = TRUE)
...
save_as_docx("Minadj_table_agegroup" = flextab_minadj2, "minad_tab" = minadj_tab, "Model Tab" = model_tab, path = docx_file)
