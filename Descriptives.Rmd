...
#Leaving Only Unique Vars based on PATIENT ID and HOSPITAL ID
#Keep all leaves all columns

#
dfor %>% distinct(PATIENT_ID,SUBJID, .keep_all = TRUE)

#
dfor[duplicated(dfor), ]
...
dft1 <- dft1 %>% mutate(age_group = case_when(AGEi > 64 ~ '>64',
                                             ...
                                             AGEi < 50 ~ '<50'))
#Create psource_derived

dft1 <- dft1 %>% mutate(psource_group = case_when(PSOURCE_01 == 1 ~ "Medicare",
                                                  PSOURCE_02 == 1 ~ "Medicaid",
                                                  PSOURCE_03 == 1 ~ "Other",
                                                  PSOURCE_04 == 1 ~ "Self-Pay",
                                                  PSOURCE_05 == 1 ~ "Medicare",
                                                  PSOURCE_06 == 1 ~ "Medicaid",
                                                  PSOURCE_07 == 1 ~ "Other",
                                                  PSOURCE_09 == 1 ~ "Other"))
...
transform(dft1, DDMER_uni=ifelse(DDMERU==1, DDMER*1, 
                  ifelse(DDMERU==3, DDMER*1000, DDMER*______ )))
