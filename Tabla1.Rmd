...
tabla1 <-tabla %>% tbl_summary(by = COVCLINTRIAL, digits = list(everything() ~ 1), type = list(starts_with(("MEDHISTO")) ~ "dichotomous"), missing = "always", missing_text = "(Missing)") %>% add_p(list(all_continuous() ~ "wilcox.test", all_categorical() ~ "chisq.test")) %>% add_overall() %>% add_stat_label()
